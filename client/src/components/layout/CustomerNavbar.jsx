...
import { Layers, LogOut, FileText, CheckSquare, HelpCircle, Wrench } from 'lucide-react';
...
export const CustomerNavbar = ({ activeTab, onTabChange }) => {
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-xs">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-6">
            ...
            <nav className="hidden sm:flex items-center space-x-1 ml-4" aria-label="Customer Portal Navigation">
              ...
              <button
                type="button"
                onClick={() => onTabChange && onTabChange('wizard')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                  activeTab === 'wizard'
                    ? 'bg-cyan-50 text-cyan-800 border border-cyan-100'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
      ...
      {/* Mobile subnav */}
      <div className="sm:hidden border-t border-slate-100 px-3 py-2 flex items-center justify-around bg-slate-50 text-[11px]">
        <button
          type="button"
          onClick={() => onTabChange && onTabChange('submit')}
        ...
        <button
          ...
          onClick={() => onTabChange && onTabChange('wizard')}
          className={`py-1 px-2 rounded font-medium ${
            activeTab === 'wizard' ? 'bg-cyan-100 text-cyan-800' : 'text-slate-600'
          }`}
        >
          Troubleshoot
        </button>
